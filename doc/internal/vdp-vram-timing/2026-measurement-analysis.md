...
band for POINT is the single value 45 over all 26 launches, and 46 is outside it. POINT is
the only command that never writes anything.
...
accesses per unit, nor the byte/pixel distinction.
...
Text mode has no refresh chain this analysis can find -- the refresh accesses do not carry
the incrementing row address they do in the other modes -- so those captures cannot be
placed on openMSX's cycle numbering directly. They can be folded on their own line marks
instead, which leaves one unknown: the offset of that mark from cycle 0. Fitting it is then
a one-parameter search, and it is sharp.
...
For text the positions are still open -- locating them needs the refresh chain that the text
captures do not provide.
